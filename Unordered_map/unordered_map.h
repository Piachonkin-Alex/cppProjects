...
public:
    ...
    public:
        ...
        explicit operator CommonIter<false>() const {
            return CommonIter<false>(node_);
        }
...
private:
    ...
    struct ListElement {
        ...
        template<typename... Args>
        ListElement(Args &&...args) : keyVal(std::forward<Args>(args)...) {}

        ListElement(const ListElement &other) : keyVal(other.keyValue), hash(other.hash) {}

        ListElement(ListElement &&other) noexcept: keyVal(std::move(other.keyValue)), hash(other.hash) {}
    };

    static const size_t DEFAULT_CAPACITY = 8;
    constexpr static const float DEFAULT_LOAD_FACTOR = 0.9;
    constexpr static const float DEFAULT_EXPAND_FACTOR = 2;


    using AllocTraits = std::allocator_traits<Alloc>;
    using NodeAllocTraits = typename Alloc::template rebind<ListElement>::other;
    using iter_type = typename List<ListElement, NodeAllocTraits>::iterator;
    using IterTraits = typename Alloc::template rebind<iter_type>::other;
    Hash hasher_;
    Equal equaler_;
    Alloc alloc_;
    List<ListElement, NodeAllocTraits> elements_;
    std::vector<typename List<ListElement, NodeAllocTraits>::iterator, IterTraits> buckets_;


    template<bool IsConst>
    class CommonIter {
    private:
        using ListIter = std::conditional_t<IsConst, typename List<ListElement, NodeAllocTraits>::const_iterator,
                typename List<ListElement, NodeAllocTraits>::iterator>;
        ListIter iter_;
    ...
    public:
        ...
        CommonIter(CommonIter &other) = default;

        explicit operator CommonIter<false>() const {
            return CommonIter<false>(static_cast<typename List<ListElement, NodeAllocTraits>::iterator>(iter_));
        }

        explicit CommonIter(ListIter iter) : iter_(iter) {}

        operator CommonIter<true>() const {
            return CommonIter<true>(iter_);
        }
        ...
        CommonIter operator++(int) {
            return CommonIter(iter_++);
        }

        bool operator==(const CommonIter &that) const {
            return iter_ == iter_;
        }
    ...
    size_t getPos(size_t hash) const {
...
public:
    ...
    UnorderedMap() : elements_(alloc_), buckets_(DEFAULT_CAPACITY, alloc_) {}

    UnorderedMap(const UnorderedMap &other) : hasher_(other.hasher_), equaler_(other.equaler_),
                                              alloc_(AllocTraits::select_on_container_copy_construction(other.alloc_)),
                                              elements_(alloc_), buckets_(DEFAULT_CAPACITY, alloc_) {

    }

    iterator find(const Key &key) {
        return static_cast<iterator>(find_(key));
    }
    ...
    void erase(const_iterator lhs, const_iterator rhs) {
...
private:
    const_iterator find_(const Key &key) {
